...
@section('header', 'Editar Especialidad')
...
<div class="max-w-2xl mx-auto space-y-5">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="bg-cyan-700 px-6 py-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-white font-semibold">{{ $especialidad->nombre }}</p>
                <p class="text-cyan-200 text-xs">Actualiza la información de esta especialidad.</p>
            </div>
        </div>

        <form action="{{ route('admin.especialidades.update', $especialidad->id) }}" method="POST" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="descripcion" rows="4"
                    class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100">{{ old('descripcion', $especialidad->descripcion) }}</textarea>
                @error('descripcion')
                <span class="text-red-600 text-xs mt-1 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="estado"
                    class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100">
                    <option value="activo" {{ old('estado', $especialidad->estado) === 'activo' ? 'selected' : '' }}>Activo</option>
                    <option value="inactivo" {{ old('estado', $especialidad->estado) === 'inactivo' ? 'selected' : '' }}>Inactivo</option>
                </select>
                @error('estado')
                <span class="text-red-600 text-xs mt-1 block">{{ $message }}</span>
                @enderror
            ...
            <div class="flex gap-3 pt-2 border-t border-gray-100">
                <button type="submit"
                    ...
                    <i class="fa-solid fa-floppy-disk mr-2"></i>Actualizar especialidad
                </button>
